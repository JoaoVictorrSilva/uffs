...
//style
import "./StyleAtualizarEmprestimo.css";
...
function AtualizarEmprestimo() {
    ...
    function handleCancelClick() {
        if (idL !== "" || matr !== "") {
            setMessageText("Cadastro da devolução cancelado!");
    ...
    async function handleSubmit() {
        if (idL !== "" && matr !== "") {
            try {
                await axios.put("/emprestimo/devolucao", {
                    matricula: matr,
                    idLivro: idL,
                });
                console.log(`Nome: ${idL} - Matricula: ${matr}`);
                setMessageText("Devolução realizada com sucesso!");
                setMessageSeverity("success");
                clearForm();
            } catch (error) {
                console.log(error);
                setMessageText("Falha no cadastro da devolução!");
